...
class FFTAnalyser(QtCore.QThread):
    ...
    def __init__(self, player: QMediaPlayer):  # noqa: F821
        super().__init__()
        self.player = player
        self.reset_media()
        self.player.sourceChanged.connect(self.reset_media)
    ...
    def reset_media(self):
        ...
        audio_file = ("C:" + self.player.source().path()).replace("/", "\\")
        print(audio_file)
    ...
    def calculate_amps(self):
        ...
        for n, amp in enumerate(point_samples):
            ...
            if (self.points[n] > 0 and amp < self.points[n] or
                    self.player.playbackState() in (self.player.PlaybackState.PausedState, self.player.PlaybackState.StoppedState)):
