...
class TrackTableItemDelegate(QStyledItemDelegate):
    selectionColorChanged =pyqtSignal(QColor, arguments=["color"])

    def __init__(self, parent: QTableView = None):
        super().__init__(parent)
        self._selection_color: QColor = SELECTION_QCOLOR
    
    @pyqtSlot(QColor)
    def setSelectionColor(self, color:QColor)->None:
        self._selection_color = QColor(color)
        self.selectionColorChanged.emit(color)
    @pyqtSlot(result=QColor)
    def getSelectionColor(self) -> QColor:
        return QColor(self._selection_color)
    
    selectionColor = pyqtProperty(QColor, getSelectionColor, setSelectionColor, notify=selectionColorChanged)
...
class TrackTableView(QTableView):
    ...
    def __init__(self, parent=None):
        ...
        self._animation = QtCore.QPropertyAnimation(self._table_delegate, b"selectionColor", self)
        self._animation.setStartValue(LOST_FOCUS_QCOLOR)
        self._animation.setEndValue(SELECTION_QCOLOR)
        self._animation.setDuration(200)
        self._table_delegate.selectionColorChanged.connect(self._repaint)
    
    @pyqtSlot()
    def _repaint(self):
        topLeft = self.model().index(0, 0)
        bottomRight = self.model().index(self.model().rowCount(), self.model().columnCount())
        self.model().dataChanged.emit(topLeft, bottomRight, [QtCore.Qt.ItemDataRole.BackgroundRole])
